...
export async function updateMemberAction(memberId: string, formData: FormData) {
  ...
  const [existing] = await db
    .select({ id: members.id, userId: members.userId, email: members.email, documentNumber: members.documentNumber })
    .from(members)
  ...
  try {
    ...
    if (email && !existing.userId && email !== existing.email) {
      ...
      const { data: authData, error: authError } = await adminSupabase.auth.admin.createUser({
        email,
        password: existing.documentNumber,
        email_confirm: true,
        app_metadata: { role: 'member' },
        user_metadata: { full_name: fullName },
      })
    ...
    await db
      .update(members)
      .set({ fullName, documentNumber, phone, email, birthDate, joinedAt, active, userId, notes })
...
export async function updateMemberNotesAction(memberId: string, formData: FormData) {
  const ctx = await requireGymAdmin()
  if (!ctx) return { error: 'No autorizado' }
  ...
  await db.update(members).set({ notes }).where(eq(members.id, memberId))
  revalidatePath(`/admin/members/${memberId}`)
  return { success: true }
}
